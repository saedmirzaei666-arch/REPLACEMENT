...
export default function TestApp() {
  ...
  const totalListeningSteps = listeningMCQs.length;
  ...
  const finishTest = useCallback(() => {
    ...
    const writingScore = estimateWritingScore(rephraseText, shortWriting);
    const speakingScore = estimateSpeakingScore(speakingBlobs.map(Boolean));

    const skills: SkillScore[] = [readingScore, listeningScore, writingScore, speakingScore];
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
          ...
          <p className="text-slate-600 mb-6">
            تعیین سطح زبان انگلیسی – چهار مهارته (Reading, Listening, Writing, Speaking)
          </p>

          <div className="space-y-3 text-sm text-slate-700 mb-8">
            <p>⏱️ مدت زمان تقریبی: <strong>۱۵ تا ۲۵ دقیقه</strong></p>
            <p>📊 سطح‌بندی بر اساس CEFR (A1 تا C2)</p>
            <p>✅ بخش‌های Reading و Listening بلافاصله نمره‌دهی می‌شوند</p>
            <p>🎤 برای Speaking به میکروفون نیاز دارید</p>
            <p>📱 روی موبایل و دسکتاپ کار می‌کند</p>
          </div>

          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800 mb-8">
            <strong>نکته:</strong> فایل‌های صوتی Listening در حال حاضر placeholder هستند.
            بعد از دیپلوی، فایل‌های واقعی را در پوشه <code>public/audio/</code> قرار دهید.
          </div>
  ...
  if (section === "listening") {
    const currentQ = listeningMCQs[listeningStep];
    return (
      <div className="max-w-3xl mx-auto px-4 py-8">
        <ProgressBar
          current={listeningStep + 1}
          total={totalListeningSteps}
          sectionName="Listening"
        />

        <div className="bg-white rounded-2xl shadow border p-6 space-y-6">
          <h2 className="text-xl font-bold">
            Listening – Question {listeningStep + 1} of {totalListeningSteps}
          </h2>
          <p className="text-sm text-slate-500">
            Listen to the audio and answer the question.
          </p>

          <AudioPlayer src={currentQ.audioUrl} maxReplays={1} />
          ...
          <div className="grid gap-2">
            {currentQ.options.map((opt, idx) => (
              <label
                key={idx}
                className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition ${
                  listeningMcqAnswers[currentQ.id] === idx
                    ? "border-primary-500 bg-primary-50"
                    : "border-slate-200 hover:border-slate-300"
                }`}
              >
                <input
                  type="radio"
                  name={currentQ.id}
                  checked={listeningMcqAnswers[currentQ.id] === idx}
                  onChange={() =>
          ...
          <div className="flex gap-3 pt-2">
            {listeningStep > 0 && (
              <button
                onClick={() => setListeningStep((s) => s - 1)}
                className="px-5 py-2.5 border border-slate-300 rounded-xl hover:bg-slate-50"
              >
                ← Back
              </button>
            ...
            <button
              onClick={() => {
                if (listeningStep < listeningMCQs.length - 1) {
                  setListeningStep((s) => s + 1);
                } else {
                  goNextSection();
                }
              }}
              disabled={listeningMcqAnswers[currentQ.id] === undefined}
              className="flex-1 py-2.5 bg-primary-600 text-white rounded-xl font-medium disabled:opacity-40 hover:bg-primary-700"
            >
              {listeningStep < listeningMCQs.length - 1
                ? "Next →"
                : "Finish Listening → Writing"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        <div className="bg-white rounded-2xl shadow-lg border p-8 space-y-8">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
            <strong>Note:</strong> Writing and Speaking scores are estimated heuristically.
            For production, connect an AI API (OpenAI / Claude) or human review for accurate evaluation.
            Audio recordings are kept only in browser memory in this demo.
          </div>
