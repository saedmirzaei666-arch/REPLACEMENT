...
## Send completed tests to Telegram

1. Create a bot with `@BotFather` and copy its token.
2. Send one message to the bot from the Telegram account or group that should receive test submissions.
3. Find that destination's numeric chat ID and copy `.env.example` to `.env.local`.
4. Set `TELEGRAM_BOT_TOKEN` and `TELEGRAM_ADMIN_CHAT_ID`, then restart the app.

The bot token is used only by the server. Never add `.env.local` to Git or expose the token in browser code.
